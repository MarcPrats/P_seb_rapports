...
On vient de tout brancher, la box s'allume (père):
\begin{enumerate}

\item Lecture de la BDD (noms capteurs)
\item Récupère la trame T°, Débit sans fil

\begin{itemize}
\item Si \textbf{ID connu} \fbox{id cpt1, cpt1, 20°C}
\begin{itemize}
\item Envoi de la trame à la BDD \fbox{id, ts,val}
\end{itemize} 
\end{itemize}
...
\item Envoi de la trame à la BDD \fbox{id, cpt2,val} 
\item Envoi de la trame à la BDD \fbox{id, ts,val}
\end{itemize}
\end{itemize}

\item Fork (fils1)
\item Trame P\_USB sous échantillonnage pour écrire dans la BDD
\item Fork (fils2)
\item Reçoit P et fils1
\item Algo interprétation

\begin{itemize}
\item Si \textbf{Doute} 
\begin{itemize}
\item Lecture BDD(T,P)
\item Si la BDD ne permet pas d'aider à différencier -> motif: proposer un éventail possible d'appareils
\item Ecriture BDD doute \fbox{P1,id doute 1, P2, id doute 2, P3,id doute 3,val}
\end{itemize} 
\end{itemize}
\begin{itemize}
\item Si \textbf{Pas de doute} 
\begin{itemize}
\item Ecriture BDD event "Allumage R1, P1"
\end{itemize} 
\item Si le client aide: écriture dans la BDD et interprétation
\end{itemize}
